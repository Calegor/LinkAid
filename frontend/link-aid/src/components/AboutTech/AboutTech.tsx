...
import { 
  ...
  FileCode, X, Zap, Folder, Terminal
} from "lucide-react";
...
const AboutTech = () => {
  ...
  const activeTech = technologiesData.find((t) => t.id === selectedId) || technologiesData[0];
  ...
  return (
    <section className="w-full py-16 lg:py-24 bg-white overflow-hidden relative">
      <div className="container mx-auto px-4 max-w-[1400px]">
        ...
        {/* 💻 VS CODE INTERFACE COM TÍTULO INTEGRADO */}
        <div className="relative pt-8 lg:pt-14">
          
          {/* 🏷️ TAG DE TÍTULO: Menor no Mobile (py-2, text-sm) e ideal no Desktop */}
          ...
          {/* 🛠️ EDITOR: h-[500px] no mobile (mais compacto) */}
          <div className="w-full h-[500px] lg:h-[650px] rounded-xl border border-slate-200 shadow-2xl bg-white flex overflow-hidden relative z-10">
            ...
            <div className="w-10 lg:w-16 bg-[#F3F3F3] border-r border-slate-200 flex flex-col items-center py-4 justify-between shrink-0 z-[50]">
              <div className="flex flex-col gap-6 items-center w-full ">
                ...
                <Search size={20} className="text-slate-300 opacity-40 cursor-not-allowed lg:w-[24px]" />
                <GitBranch size={20} className="text-slate-300 opacity-40 cursor-not-allowed lg:w-[24px]" />
                <Play size={20} className="text-slate-300 opacity-40 cursor-not-allowed lg:w-[24px]" />
              </div>
            ...
            `}>
              <div className="px-4 py-3 flex items-center justify-between">
                <span className="text-[10px] font-bold uppercase text-slate-500 tracking-wider">Explorer</span>
                <X size={14} className="lg:hidden cursor-pointer text-slate-400" onClick={() => setIsSidebarOpen(false)} />
              </div>
              <div className="flex items-center gap-1 px-2 py-1 bg-slate-200/50">
                <ChevronDown size={14} className="text-slate-600" />
                <span className="text-[10px] font-bold text-slate-700 uppercase">Link-Aid</span>
              </div>
              <div className="flex flex-col mt-1">
                <div className="flex items-center gap-1 px-4 py-1">
                  <Folder size={14} className="text-blue-400 fill-blue-400/20" />
                  <span className="text-[12px] text-slate-600 font-medium">technologies</span>
                </div>
                  ...
                  {technologiesData.map((tech) => (
                    <button
                      ...
                      className={`flex items-center gap-2 pl-10 pr-4 py-1.5 transition-colors text-left ${
                        selectedId === tech.id ? "bg-blue-100/50 text-blue-600" : "text-slate-500 hover:bg-slate-200/30 cursor-pointer"
                      }`}
                    >
                      <FileCode size={14} className={selectedId === tech.id ? "text-blue-500" : "text-slate-400"} />
                      <span className="text-[12px] truncate cursor-pointer">{tech.name}.tsx</span>
                    </button>
                  ))}
                </div>
              </div>
            ...
            <div className="flex-grow flex flex-col bg-white overflow-hidden z-10">
              <div className="h-9 bg-[#F3F3F3] flex items-center border-b border-slate-200">
                <div className="bg-white h-full px-4 border-r border-slate-200 flex items-center gap-2 min-w-[120px] lg:min-w-[140px] shadow-[0_-2px_0_inset_#2563eb]">
                  ...
                  <span className="text-[11px] lg:text-[12px] text-slate-700 font-medium">{activeTech.name}.tsx</span>
                  <X size={12} className="ml-auto text-slate-400" />
                </div>
              </div>

              <div className="flex-grow flex overflow-hidden bg-white" ref={editorRef}>
                <div className="hidden sm:flex w-10 pt-6 bg-[#FDFDFD] border-r border-slate-100 flex-col items-center gap-1">
                  {[...Array(15)].map((_, i) => (
                    <span key={i} className="text-[10px] font-mono text-slate-200">{i + 1}</span>
                  ))}
                </div>
                ...
                <div className="flex-grow p-5 lg:p-12 overflow-y-auto">
                  <div className="max-w-2xl mx-auto lg:mx-0">
                      ...
                      <div className="mb-6 lg:mb-8 p-4 lg:p-6 rounded-2xl bg-slate-50/50 border border-slate-100 flex flex-col sm:flex-row items-center gap-4 lg:gap-6">
                          <img src={activeTech.icon} alt="" className="w-16 h-16 lg:w-24 lg:h-24 object-contain" />
                      ...
                      <div className="space-y-6">
                          ...
                          <div className="pt-4 lg:pt-6 border-t border-slate-100">
                              <p className="text-[9px] font-mono text-slate-400 mb-3 uppercase tracking-widest">{"// tags"}</p>
                              <div className="flex flex-wrap gap-2">
                                  {activeTech.labels.map((label, idx) => (
                                      <span key={idx} className="px-2 py-1 bg-blue-50/50 text-blue-600 border border-blue-100 rounded-lg font-mono text-[9px] uppercase">
                                          {label}
                                      </span>
                                  ))}
                              </div>
